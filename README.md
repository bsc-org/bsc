...
- `section:` rendert eine komplette Sektion (`section_id`), `name:` einen einzelnen Parameter (globale Namenssuche); Reihenfolge der Zeilen bestimmt die Anzeige
...
`type_map.json` aus `WebSettings.h` und extrahiert das gescopte CSS-Subset aus
`css/style.css` der WebApp. Bei Firmware-/WebApp-Änderungen erneut ausführen
und das Ergebnis committen. Die Daten liegen bewusst außerhalb von `docs/` und
werden nicht Teil von `site/`.
...
Bestehende `bsc_content`-HTML-Blöcke bleiben unverändert funktionsfähig und
können nach und nach durch Marker ersetzt werden.
